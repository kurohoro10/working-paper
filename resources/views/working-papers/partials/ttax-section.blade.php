...
<div class="space-y-6">
    ...
    <div class="border-b border-gray-200">
        <nav class="-mb-px flex space-x-8">
            <button type="button" class="quarter-tab-ttax active border-b-2 border-pink-600 py-4 px-1 text-sm font-medium text-pink-600" data-quarter="all">
                All Periods
            </button>
            <button type="button" class="quarter-tab-ttax border-b-2 border-transparent py-4 px-1 text-sm font-medium text-gray-500 hover:text-gray-700 hover:border-gray-300" data-quarter="q1">
                Q1 (Jul-Sep)
            </button>
            <button type="button" class="quarter-tab-ttax border-b-2 border-transparent py-4 px-1 text-sm font-medium text-gray-500 hover:text-gray-700 hover:border-gray-300" data-quarter="q2">
                Q2 (Oct-Dec)
            </button>
            <button type="button" class="quarter-tab-ttax border-b-2 border-transparent py-4 px-1 text-sm font-medium text-gray-500 hover:text-gray-700 hover:border-gray-300" data-quarter="q3">
                Q3 (Jan-Mar)
            </button>
            <button type="button" class="quarter-tab-ttax border-b-2 border-transparent py-4 px-1 text-sm font-medium text-gray-500 hover:text-gray-700 hover:border-gray-300" data-quarter="q4">
                Q4 (Apr-Jun)
            </button>
        </nav>
    </div>
    ...
    <div>
        <div class="flex justify-between items-center mb-3">
            ...
            <button type="button" class="text-sm text-pink-600 hover:text-pink-700 font-medium">
                + Add Income
            </button>
        </div>
    ...
    {{-- Summary Card --}}
    <div class="bg-pink-100 rounded-lg p-6 border-2 border-pink-300">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
                ...
                <div class="text-2xl font-bold text-pink-600">$0.00</div>
            </div>
        </div>
    </div>
...
@push('scripts')
<script>
    // Quarter tab switching for TTax
    document.querySelectorAll('.quarter-tab-ttax').forEach(tab => {
        tab.addEventListener('click', function() {
            document.querySelectorAll('.quarter-tab-ttax').forEach(t => {
                t.classList.remove('active', 'border-pink-600', 'text-pink-600');
                t.classList.add('border-transparent', 'text-gray-500');
            });

            this.classList.add('active', 'border-pink-600', 'text-pink-600');
            this.classList.remove('border-transparent', 'text-gray-500');

            const quarter = this.dataset.quarter;
            console.log('TTax - Filtering by quarter:', quarter);
        });
    });
</script>
@endpush
